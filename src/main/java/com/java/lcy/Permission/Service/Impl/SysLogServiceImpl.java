...
import com.java.lcy.Permission.Mapper.*;
...
public class SysLogServiceImpl implements SysLogService {
    ...
    public void recover(int id) {
        ...
        Preconditions.checkNotNull(sysLog, ResultEnum.RESTORE_RECORD_NOT_EXIST.getMessage());//"待还原的记录不存在"
        switch (sysLog.getType()) {
            case LogType.TYPE_DEPT:
                SysDept beforeDept = sysDeptMapper.selectByPrimaryKey(sysLog.getTargetId());
                Preconditions.checkNotNull(beforeDept, ResultEnum.RESTORE_DEPT_NOT_EXIST.getMessage());//待还原的部门已经不存在了
                if (StringUtils.isBlank(sysLog.getNewValue()) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException(ResultEnum.SAVE_AND_UPDATE_CANNOT_RESTORE);//新增和删除操作不做还原
                }
            ...
            case LogType.TYPE_USER:
                ...
                Preconditions.checkNotNull(beforeUser, ResultEnum.RESTORE_USER_NOT_EXIST.getMessage());//"待还原的用户已经不存在了"
                if (StringUtils.isBlank(sysLog.getNewValue()) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException(ResultEnum.SAVE_AND_UPDATE_CANNOT_RESTORE);
                }
            ...
            case LogType.TYPE_ACL_MODULE:
                ...
                Preconditions.checkNotNull(beforeAclModule, ResultEnum.RESTORE_ACL_MODULE_NOT_EXIST.getMessage());//待还原的权限模块已经不存在了
                if (StringUtils.isBlank(sysLog.getNewValue()) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException(ResultEnum.SAVE_AND_UPDATE_CANNOT_RESTORE);
                }
            ...
            case LogType.TYPE_ACL:
                ...
                Preconditions.checkNotNull(beforeAcl, ResultEnum.RESTORE_ACL_NOT_EXIST.getMessage());//待还原的权限点已经不存在了
                if (StringUtils.isBlank(sysLog.getNewValue()) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException(ResultEnum.SAVE_AND_UPDATE_CANNOT_RESTORE);
                }
            ...
            case LogType.TYPE_ROLE:
                ...
                Preconditions.checkNotNull(beforeRole, ResultEnum.RESTORE_ROLE_NOT_EXIST.getMessage());//待还原的角色已经不存在了
                if (StringUtils.isBlank(sysLog.getNewValue()) || StringUtils.isBlank(sysLog.getOldValue())) {
                    throw new ParamException(ResultEnum.SAVE_AND_UPDATE_CANNOT_RESTORE);
                }
            ...
            case LogType.TYPE_ROLE_ACL:
                ...
                Preconditions.checkNotNull(aclRole, ResultEnum.ROLE_NOT_EXIST.getMessage());//"角色已经不存在了"
            ...
            case LogType.TYPE_ROLE_USER:
                ...
                Preconditions.checkNotNull(userRole, ResultEnum.ROLE_NOT_EXIST.getMessage());
    ...
    public PageResult<SysLogWithBLOBs> searchPageList(SearchLogParam param, PageQuery page) {
        ...
        try {
            ...
            throw new ParamException(ResultEnum.DATE_FORMAT_ERROR);//传入的日期格式有问题，正确格式为：yyyy-MM-dd HH:mm:ss
        }
    ...
    public void saveDeptLog(SysDept before, SysDept after) {
        SysLogWithBLOBs sysLog = saveObject(before, after, LogType.TYPE_DEPT);
    ...
    public void saveUserLog(SysUser before, SysUser after) {
        SysLogWithBLOBs sysLog = saveObject(before, after, LogType.TYPE_USER);
        sysLog.setTargetId(after == null ? before.getId() : after.getId());
        sysLogMapper.insertSelective(sysLog);
    }
    ...
    public void saveAclModuleLog(SysAclModule before, SysAclModule after) {
        SysLogWithBLOBs sysLog = saveObject(before, after, LogType.TYPE_ACL_MODULE);
        sysLog.setTargetId(after == null ? before.getId() : after.getId());
        sysLogMapper.insertSelective(sysLog);
    }
    ...
    public void saveAclLog(SysAcl before, SysAcl after) {
        SysLogWithBLOBs sysLog = saveObject(before, after, LogType.TYPE_ACL);
        sysLog.setTargetId(after == null ? before.getId() : after.getId());
        sysLogMapper.insertSelective(sysLog);
    }
    ...
    public void saveRoleLog(SysRole before, SysRole after) {
        SysLogWithBLOBs sysLog = saveObject(before, after, LogType.TYPE_ROLE);
        sysLog.setTargetId(after == null ? before.getId() : after.getId());
        sysLogMapper.insertSelective(sysLog);
    }

    @Transactional
    <T> SysLogWithBLOBs saveObject(T before, T after, int type) {
